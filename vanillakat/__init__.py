...
TopicChannel = TextChannel | StageChannel

logging.basicConfig(level=logging.INFO)


def clean_content(message):
    return utils.escape_mentions(utils.escape_markdown(message))


class VanillaKat(commands.Bot):
    def __init__(
        self,
    ) -> None:
        ...
        super().__init__(
            command_prefix=os.getenv("PREFIX") or str(uuid.uuid4()), intents=intents
        )
    ...
    @lru_cache
    async def atsumori(self) -> Emoji:
        await self.wait_until_ready()
        assert self.user

        guild = utils.get(self.guilds, owner_id=self.user.id)
        if not guild:
            guild = await self.create_guild(name=f"{self.user.name}'s Server")

        if emoji := utils.get(guild.emojis, name="atsumori"):
            return emoji
        else:
            with open(os.path.join(__file__, "assets/atsumori.png"), "rb") as f:
                return await guild.create_custom_emoji(name="atsumori", image=f.read())

    async def on_message(self, message: Message) -> None:
        if not self.user or message.author.bot or message.author == self.user:
            return
        ...
        if message.content.endswith("だよね？"):
            ...
            message.content == "偽物のポケモン作って"
            or message.content == "偽物のpokemon作って"
        ):
            embed = Embed(
            ...
            c = [
                ...
                "ま、、まさか、、",
                "デフ！",
                "Error:`チョットナニイッテルカワカリマセンデシタ`",
            ]
            choice = random.choice(c)
            await message.channel.send(choice)
        elif message.content == "うるさい":
            await message.channel.send(
                "言葉をつつしみたまえ！！君はおこちゃま大魔王の前にいるのだ！！"
            )
            ...
            choice = random.choice(c)
            await message.channel.send(
                "{}我こそが本当の大いなるお子ちゃま大魔王様であるぞ！".format(choice)
            )
        elif message.content == "さようなら":
            await message.channel.send("さようなら")
        elif "ダメージを受けた" in message.content:
            await message.channel.send(
                "グハ！お主、なかなかやるな。ただものではないな！何者だ！まさか勇者だと！"
            )
        elif "誰もいない" in message.content:
            await message.channel.send(
                "ねえそういえば話変わるけど誰もいないといえばクラスの女子から「今家に誰もいないから来て」って言われていったらその女子もいなかった"
            )
            return
        elif "留守番" in message.content:
            await message.channel.send(
                "ねえそういえばさぁ。話変わるけどこの前留守番と言えば一人息子がちゃんと留守番できているかどうか、公衆電話から他人のふりをして家に電話してみた。"
            )
            await message.channel.send(
                "俺「もしもし、お母さんいる？」\n息子「いらない」\nって言われた"
            )
            ...
            await message.channel.send(
                "あ。そういえば話変わるけど詐欺と言えば昨日うちのじーちゃんが「詐欺商法にだまされない本」という本を３０万円で買ってきた"
            )
            return
        elif "ボケ防止" in message.content:
            await message.channel.send(
                "ねえ、話変わるけどボケと言えば、昨日じいちゃんが「ボケないための本」を買ってきた\n今日も買ってきた"
            )
            return
            ...
            if message.author.bot:
                return
            await message.channel.send(
                "話変わるけどバス停と言えばかつて、バス停を毎日ちょっとずつ家の方に近づけていた。"
            )
            ...
            if message.author.bot:
                return
            await message.channel.send(
                "話変わるけどチキンと言えばミニストップでファーストフード頼んだ時に\n「骨なしチキンのお客様ー！」って呼び出されたんだけど、、、"
            )
            await asyncio.sleep(1.5)
            await message.channel.send(
            ...
            c = [
                "悪いなのび太、このゲームは3人までなんだ",
                "先生！生徒に暴力をふるってはいけません！",
                "美しいということばは、ぼくのためにあるんだなあ。",
            ]
            choice = random.choice(c)
            embed = Embed(
                title=choice, description=(), color=Color(random.randint(0, 0xFFFFFF))
            )
            embed.set_author(
                name="骨川スネ夫",
                icon_url="https://pbs.twimg.com/profile_images/681367744416059392/f9eja41S_400x400.jpg",
            )
            await message.channel.send(embed=embed)
            ...
            embed.set_author(
                name="剛田武",
                icon_url="http://makisimamu.xyz/wp-content/uploads/2017/01/aa-109.png",
            )
            await message.channel.send(embed=embed)
        ...
        if message.content == "ガード！":
            ...
            await message.channel.send(
                "駐車場と言えばだいぶ前に某飲食店の「お客様の声」に、駐車場を増やして欲しいです、と書いておいた。"
            )
            await message.channel.send(
                "後日その店を訪れると、店のあった場所に広い駐車場ができていた。"
            )
        elif message.content == "チャンネルトピックは？":
            if isinstance(message.channel, TopicChannel):
                await message.channel.send(
                    "```{}```".format(clean_content(message.channel.topic))
                )
            else:
                await message.channel.send("トピックねぇよ")
            ...
            await message.channel.send(
                "ギャンブルと言えば友達が「俺はもう絶対にギャンブルなんかしねぇんだ。賭けてもいいぜ！！」って言ってた"
            )
        elif message.content == "新幹線":
            await message.channel.send(
                "新幹線と言えば、新幹線に��めて乗った時、座席を倒そうと手元のボタンを押したら隣のおっさんが倒れていった"
            )
        elif message.content == "インド":
            await message.channel.send(
                "「インドの人って右手でごはん食べるんだって」\n「えっ！口じゃねーんだ！！」"
            )
        elif message.content == "まさか！":
            await message.channel.send("フハハハハ  地獄の底から這い上がって来たぞ")
        elif (
            message.content == "よろしくおねがいします"
            or message.content == "よろしくお願いします"
        ):
            c2 = random.choice(
                [
                    "ボンジュールでごわす",
        ...
        if message.content == "RIP":
            ...
            await message.channel.send(
                "カレーと言えばこの前この前携帯電話がカレーに刺さった。あまりにも見事に直立していたので、記念に撮ろうと思って携帯探したらカレーに刺さってた。"
            )
        elif message.content == "病院":
            a = [
                "病院と言えばこの前病院でかゆみ止めの薬をもらった。 説明書を見たら副作用に「かゆみ」と書いてあった。 効くのか、これ"
            ]
            if ch := self.get_channel(583932805340463104):
                assert isinstance(ch, TopicChannel)
            ...
            dog = [
                "飼ってる犬が脱走した。無事に帰ってきたのは良かったけど、おでこに「ネコ」って落書きされてた",
            ...
            await message.channel.send(
                "話変わるけどコンビニといえばこの前{}".format(choice)
            )
            return
            ...
            await message.channel.send(
                "ルンバといえばこの前ばあちゃんがルンバに餌やってた"
            )
            return
        elif "迷惑" in message.content:
            await message.channel.send(
                "迷惑といえば迷惑メールフィルターを強にしてるのに上司からメール来る"
            )
            return
        elif "Excel" in message.content:
            await message.channel.send(
                "Excelといえば部長がExcelのことをエグザイルって言う…"
            )
            return
        elif message.content == "Unknown" or message.content == "unknown":
            await message.channel.send(
                "Unknownといえば後輩がUnknownを「うんこなう」って読んでたのでもう帰りたい。"
            )
        elif message.content.startswith("クエ") or message.content.startswith(
            "ぐええええ"
        ):
            kue = [
                "お子ちゃまは寝るわ",
                "クワックワックワックワックワックワックワックワックワッ",
                "クエー",
                "くええええ",
                "くえーー",
                "モー",
            ...
            if message.guild:
                if kue := utils.get(message.guild.text_channels, name="kue-count"):
                    next = int(kue.topic or 0) + 1
                    await kue.edit(topic=f"{next}")
            ...
            k3 = [
                ...
                "こんちゃあ",
                "くうぇぇ",
                "グハッ！貴様なかなかやるな！あ、、こんちゃ",
                "ハローここに来たから生きて帰れると思うなw",
            ]
            if 18 <= time <= 5:
            ...
            message.content.startswith("おはよ")
            or message.content == "おは"
            or message.content == "おっはー"
            or message.content == "おっは"
            or message.content == "おっはよ"
            or message.content == "おっはよ～"
        ):
            p = datetime.now().hour + 9  # JST?
            oha = [
                "おはようございます。朝日がきれいですね。",
                "おはよ",
                "おはよう",
            ...
            oha2 = [
                ...
                "おそよう",
                "起きるの遅いですね",
                "おはようございます。太陽が輝いてますね。もう{}時です".format(p),
            ]
            oha3 = [
                "遅いですね",
                "太陽傾いてるけど",
            ...
            if 6 <= h < 8:
                ...
                await message.channel.send(
                    "{}時だけどおそよう。君の一日はこれからかな？".format(h)
                )
            elif 20 <= h < 24:
                await message.channel.send("・・・もう{}時だから寝ようね".format(h))
            elif 1 <= h < 5:
                await message.channel.send(
                    "{}時に起きるって早起きなのか寝坊なのか".format(h)
                )
            else:
                c = [
                    "おそよお！",
            ...
            ne3 = [
                ...
                "昼だけど",
                "クエー",
                "今{}時だよ".format(datetime.now().hour),
            ]
            ...
            await message.channel.send(message.author.display_avatar.url)
        elif message.content.startswith("サーバーのアイコン"):
            if message.guild:
                await message.channel.send(
                    message.guild.icon.url
                    if message.guild.icon
                    else "このサーバーアイコンないんだが"
                )
            else:
                await message.channel.send("ここDMなんだが")
        elif message.content in ["こんにちは", "こん", "kon", "konn"]:
            he = [
                "こん",
                "こんにちは",
                f"{clean_content(message.author.display_name)}さん、こんにちは",
                "こんにちは今日はいい天気ドスな",
                "クエー",
                "さぁゲームを始めようか",
                "地獄の底から這い上がって来たぞ",
            ...
            if message.author.bot:
                return
            await message.channel.send(
                "ふはははっはは　ひれふせ下民どもぉ{}様のおとおりだあ".format(
                    self.user.mention
                )
            )
            ...
            if datetime.now().month == 1:
                if datetime.now().day == 1:
                    nw = [
                        "元日おめでとう",
                        "お年玉くれ",
                        "{}年もよろしく！".format(datetime.now().year),
                    ]
                ...
                nw = [
                    "まだだろ",
                    "まだ{}年".format(datetime.now().year),
                    "正月に近くもない。でもお年玉くれ",
                    "まだ{}月だね".format(datetime.now().month),
                ]
                choice = random.choice(nw)
                await message.channel.send(choice)
            ...
            await message.channel.send(
                "ストーカーと言えば俺の友達が毎日見守ってる女の子がストーカーにあってるらしい"
            )
            return
        elif message.content == "誕生日プレゼント":
            await message.channel.send(
                "誕プレと言えば姪の誕生日に｢これで何か美味しいものでも食べなさない。｣って割り箸あげました。"
            )
            ...
            choice = random.choice(c)
            await message.channel.send(
                "{}今は{}年の{}月{}日だね".format(choice, y, m, d)
            )
            ...
            await message.channel.send(
                "今は{}年の{}月{}日{}時{}分{}秒だね".format(y, m, d, h, mi, s)
            )
            ...
            await message.channel.send(
                "グッハ　まさかお前レベル３２のゴブリンより強いのか"
            )
        elif "田中" in message.content:
            if message.author.bot:
                return
            await message.channel.send(
                "中1の時、学年中の靴箱の「田中」を「油虫」にして 校長室に呼ばれた"
            )
            return
            ...
            if message.author.bot:
                return
            await message.channel.send(
                "日本の官公庁の中で本気で明日の日本のこと考えてるのって、どこだと思う？\n俺「気象庁位だろ」"
            )
            return
            ...
            await message.channel.send(
                "え..簡単...? {}ってまさかプロ..?".format(message.author.mention)
            )
            ...
            message.content == "疲れた"
            or message.content == "つかれた"
            or message.content == "おつ"
        ):
            c = [
                "おつかれ～",
                "お疲れさまでござる",
                ...
                "クエー",
                "お疲れ様です。",
                "{}さん、お疲れ～".format(message.author.name),
            ]
            choice = random.choice(c)
            await message.channel.send(choice)
            ...
            fakenews = [
                "校長先生のかつら",
                "某スマホゲームで課金アイテム",
                "０点のテスト答案を鼻をかむティッシュとして",
                "学校で１年ほど洗っていない上靴を",
                "テスト勉強中に携帯",
                "修学旅行で一人だけプライベートジェット",
                "友達３人とマクドナルドに行った際、一人だけクーポン",
                "ママのへそくり",
                "パパの大事なゴルフクラブ",
                "学校二階奥の教師用男子便所",
                "テスト中にカンニング方法を検索するためにスマホ",
            ]
            ...
            sigfake = [
                "あんパンを買うも、中身が入ってなかった",
                "課金アイテムを買った瞬間iPhone水没",
                "宝くじをあてるもなくす",
                "１００点をテストでとるも、なくし結局怒られる",
                "５０％引きのお得商品を急いで取ろうとして指を突き指",
            ]
            ...
            await message.channel.send(
                "ピカソの本名はたしか```パブロ・ディエゴ・ホセ・フランシスコ・デ・パウラ・ファン・ネポムセーノ・マリア・デ・ロス・レメディオス・クリスピン・クリスピアーノ・デ・ラ・サンティシマ・トリニダード・ルイス・イ・ピカソ```だったよね～。\n \
                    これ常識だよね～。"
            )
...
if __name__ == "__main__":
    if token := os.getenv("TOKEN"):
        VanillaKat().run(token)
    else:
        logging.fatal("トークンなんだ")
